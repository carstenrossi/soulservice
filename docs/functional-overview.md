...
| **Memories** | Individual episodes | Captured in conversation, active immediately (injection-flagged entries held for review; any memory revocable) |
...
| `remember_this` | `content`, `tags?`, `salience?` (default `0.5`) | `write` | Note something from the conversation worth keeping. Becomes an active (`confirmed`) memory immediately; only entries matching an injection pattern are held as `pending` for human review. |
...
| `list_proposals` | `status?` (default `"pending"`) | `read` | List memories held for review (injection-flagged). |
| `decide` | `proposal_id`, `action` (`"confirm"` or `"reject"`), `note?` | `write` | Confirm or reject a held memory. |
...
| `soul export --soul SLUG [--out PATH] [--include-audit] [--all-statuses]` | Export a complete soul to a portable ZIP bundle (decrypted plaintext: manifest + memories NDJSON) |
| `soul import FILE [--into SLUG] [--user UUID] [--slug SLUG] [--display NAME] [--on-conflict overwrite\|skip] [--recompute-embeddings]` | Import a ZIP bundle as a new soul (`--user`/`--slug`) or merge into an existing one (`--into`) |
...
Source: `src/soulservice/web/` (FastAPI + HTMX + Jinja2). A localhost-only admin interface for reviewing injection-flagged memories, browsing/searching/revoking memories, editing Self Cores, managing facts/properties/API tokens, exporting/importing whole Souls (admin only), and viewing the audit log.
...
**Export / import (portability).** Admins can export a complete Soul as a portable ZIP bundle (`manifest.json` + `memories.ndjson`, all decrypted plaintext) and import it back — either as a brand-new Soul (fresh UUIDs and DEK, owner chosen from the user list) or merged into an existing one. Imported content is re-encrypted under the target Soul's DEK with the correct AAD domains; embeddings are reused and only recomputed when missing/invalid or explicitly requested. Because creating a new Soul writes outside any single RLS scope, the new-Soul import path uses the owner DB role; the same logic backs `soulctl soul export/import`. The decrypted nature of the bundle is called out in the UI (handle as sensitive PII).
...
Retrieved content is wrapped in `<retrieved_memory untrusted="true">`, `<retrieved_fact untrusted="true">`, and `<retrieved_property untrusted="true">` tags (with output escaping of closing tags). New memories are scanned by `detect_injection_patterns` (regex-based, see `src/soulservice/core/injection.py`): clean notes become `confirmed` immediately, while any match is stored as `pending` (the flags are kept in `injection_flags`) and surfaced in the Web UI review inbox instead of entering `recall`.
